...
def request_predictions(
    ...
    csv_bytes = api_input_df.to_csv(index=False).encode("utf-8")
    files = {
        "file": (
            filename,
            csv_bytes,
    ...
    result_df = pd.DataFrame(response_data["predictions"])
...
def create_display_dataframe(
    ...
    display_df["predicted_defect"] = display_df[
        "predicted_defect"
    ].map(
        {
            0: "✅ Normal",
...
def show_prediction_results(
    ...
    """Display prediction results without Streamlit DataFrame rendering."""
...
def show_chart(defect_counts: pd.Series) -> None:
...
def main() -> None:
    ...
    st.text(raw_df.head().to_string(index=False))
    ...
    try:
        ...
        removed_rows = int(response_data.get("removed_rows", 0))
        ...
        if result_df.empty:
            st.warning("The API did not return any valid predictions.")
            return
        ...
        st.error("The prediction API request timed out.")
    except requests.exceptions.HTTPError as error:
        st.error(f"Prediction API error: {error}")
    except requests.exceptions.RequestException as error:
        st.error(f"Request failed: {error}")
    except (KeyError, TypeError, ValueError) as error:
        st.error(f"Prediction response could not be processed: {error}")
